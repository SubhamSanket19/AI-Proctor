...
def get_frame(imgData):
    # Set default safe fallback values
    mob_status = 0
    person_status = 0
    user_move1 = 0
    user_move2 = 0
    ...
    try:
        ...
        if isinstance(imgData, str):
            ...
            imgData_bytes = imgData.encode('utf-8')
        else:
            imgData_bytes = imgData

        nparr = np.frombuffer(base64.b64decode(imgData_bytes), np.uint8)
        image = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
        
        if image is None:
            print("Error: Decoded image is None", flush=True)
            return {
                'jpg_as_text': imgData,
        ...
        # 1. Object Detection (YOLOv3) for mobile/person count
        try:
            ...
            if has_mobile:
                print('Mobile Phone detected', flush=True)
                mob_status = 1
            else:
                mob_status = 0
            ...
            if count == 0:
                ...
                print('More than one person detected', flush=True)
                person_status = 2
            else:
                person_status = 0
            ...
            print(f"Error in YOLO / Object detection: {eyolo}", flush=True)
        ...
        try:
            ...
            if gaze.is_blinking():
                ...
                eye_movements = 0
        except Exception as egaze:
            print(f"Error in Gaze Tracking: {egaze}", flush=True)

        # 3. Head Pose Estimation (dlib face + landmarks)
        try:
            ...
            for face in faces:
                ...
                if ang2 >= 48:
                    print('Head right', flush=True)
                    user_move2 = 4
                elif ang2 <= -48:
                    print('Head left', flush=True)
                    user_move2 = 3
                else:
                    user_move2 = 0
        except Exception as ehead:
            print(f"Error in Head Pose estimation: {ehead}", flush=True)

        # Encode processed image back to base64
        try:
            ret_enc, jpeg = cv2.imencode('.jpg', image)
            if ret_enc:
                jpg_as_text = base64.b64encode(jpeg)
                if isinstance(jpg_as_text, bytes):
                    jpg_as_text = jpg_as_text.decode('utf-8')
        except Exception as eenc:
            print(f"Error encoding output image: {eenc}", flush=True)

    except Exception as eglobal:
        print(f"Global camera processing error: {eglobal}", flush=True)
